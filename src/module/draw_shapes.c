...
void draw_holes(FILE* fp, vector* a, vector* b, float inset){
	...
	fprintf(fp, "M %.3f %.3f ", a->x, a->y);
	fprintf(fp, "m %.3f %.3f ",
		-vu.x*MAT_IN/2 + vn.x*(1+inset)*MAT_OUT,
		-vu.y*MAT_IN/2 + vn.y*(1+inset)*MAT_OUT);

	int i;
	for(i=0; i<NUM_HOLES; i++){
		fprintf(fp, "m %.3f %.3f ", v.x/(NUM_HOLES+1), v.y/(NUM_HOLES+1));

		fprintf(fp, "l %.3f %.3f ",  vn.x*MAT_IN,  vn.y*MAT_IN); //sides 1
		fprintf(fp, "l %.3f %.3f ",  vu.x*MAT_IN,  vu.y*MAT_IN); //2
		fprintf(fp, "l %.3f %.3f ", -vn.x*MAT_IN, -vn.y*MAT_IN); //3
		fprintf(fp, "l %.3f %.3f ", -vu.x*MAT_IN, -vu.y*MAT_IN); //4
